...
def evolve_cluster(cluster,mCut,dt,tend,mCluster,rCluster):
    ...
    # Plotting results
    plot_energy_error(times.value_in(units.Myr), dE, Qs)
    #plot_cluster(low_mass_stars, high_mass_stars)


def plot_energy_error(times, dE, Qs):
    fig_ee = plt.figure(figsize=(8,6))
    ax = fig_ee.add_subplot(111)
    ax.plot(times, dE, '-o', label='dE/E0')
    ax.plot(times, Qs, '-o', label='-T/U')
    #ax.set_yscale('log')
    ax.set_xlabel('Time (Myr)')
    ax.set_ylabel('Relative Energy Error')
    ax.legend()
    plt.show()
    fig_ee.savefig('energy_error.jpg')
