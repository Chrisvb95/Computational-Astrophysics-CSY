...
def plot_cloud_cluster(cluster, sph, title, vrange=[-5,2], L=400):
    ...
    #rho = make_map(sph, L)
    #x = cluster.gas_particles.x.value_in(units.parsec)
    #y = cluster.gas_particles.y.value_in(units.parsec)
    #z = cluster.gas_particles.z.value_in(units.parsec)
    ...
    ax.scatter(cluster.x.value_in(units.parsec), cluster.y.value_in(units.parsec), s=5, c='b')   
    ax.scatter(cloud.x.value_in(units.parsec), cloud.y.value_in(units.parsec), s=1, c='r')
...
if __name__ == '__main__':
    ...
    cluster = cluster_init(N_cluster, Mtot_cluster, Rvir_cluster,\
                                              v_cluster, p_cluster)
    cloud= cloud_init(N_cloud, M_cloud, R_cloud)

    t_end = 100 | units.Myr
    dt_bridge = 0.1 | units.Myr
    dt_diag = 1.0 | units.Myr

    lr9 = evolve(cluster,cloud, t_end,dt_bridge, dt_diag)
