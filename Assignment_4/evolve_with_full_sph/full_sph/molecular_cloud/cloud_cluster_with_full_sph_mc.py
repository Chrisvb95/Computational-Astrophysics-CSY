...
if __name__ == '__main__':
    ...
    cluster, conv_grav = cluster_init(N_cluster, Mtot_cluster, Rvir_cluster, v_cluster, p_cluster)
    cloud, conv_sph = cloud_init(N_cloud, Mtot_cloud, R_cloud)
    ...
    for filename in [printout_file, 'lr_data.txt', 'E_data.txt', 'cloud_data.txt']:
        with open(filename,'a') as f:
            f.write('# Cloud: number of gas particles: %d, total mass: %d MSun, virial radius: %d pc;\n'\
                     %(N_cloud, Mtot_cloud.value_in(units.MSun), R_cloud.value_in(units.parsec)))
