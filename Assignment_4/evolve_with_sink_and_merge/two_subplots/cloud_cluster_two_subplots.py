...
def plot_cloud_cluster(cluster, sph, title, vrange=[-6,2]):
    ...
    # make scatter plot for the distribution of stars and gas particles in the first axes
    ax1.scatter(cloud.x.value_in(units.parsec), cloud.y.value_in(units.parsec), s=1, c='r',\
               label='cloud')
    for ax in [ax1,ax2]:
        ax.scatter(cluster.x.value_in(units.parsec), cluster.y.value_in(units.parsec), s=2, c='b',\
               label='cluster')
    if len(sph.dm_particles) > 0:
        stars_to_be_plotted = sph.dm_particles.select_array(\
                              lambda x,y,z: (x**2+y**2+z**2).value_in(units.parsec**2) < (L/2.)**2, ["x","y","z"])
        for ax in [ax1,ax2]:
            ax.scatter(stars_to_be_plotted.x.value_in(units.parsec),\
                       stars_to_be_plotted.y.value_in(units.parsec), s=2,\
                       marker='*', c='k', label='formed star', alpha=0.6)

    # make color map for projected cloud density in the second axes
    vmin, vmax = vrange
...
def timescale(particles, kind='dyn', unit=units.Myr, G=constants.G):
...
def evolve(cluster,cloud, converter_grav,converter_sph, t_end, dt_bridge, dt_diag,\
    ...
    dt_bridge = dt_bridge.in_(unit_time)
    ...
    for i,t in enumerate(times):
        ...
        t_dyn_cloud, t_rh_cloud = timescale(sph.gas_particles, "both", unit_time)
        t_ff_cloud = sph.gas_particles.dynamical_timescale().value_in(unit_time)
        t_dyn_cluster, t_rh_cluster = timescale(gravity.particles, "both", unit_time)
        t_ff_cluster = gravity.particles.dynamical_timescale().value_in(unit_time)
        all_timescales = [t_dyn_cloud, t_rh_cloud, t_ff_cloud, t_dyn_cluster, t_rh_cluster, t_ff_cluster] | unit_time
        # check if the bridge timestep should be lowered
        if all_timescales.amax() < 10 * dt_bridge:
              dt_bridge = all_timescales.amax() / 10.
        #with open('print_out.txt', 'a') as pf:
            #pf.write(f't_ff_cloud = {sph.gas_particles.dynamical_timescale().in_(units.Myr)}, '+\
            #         f't_dyn_cloud, t_rh_cloud = {t_dyn_cloud, t_rh_cloud} Myr\n'+\
            #         f't_ff_cluster = {gravity.particles.dynamical_timescale().in_(units.Myr)} '+\
            #         f't_dyn_cluster, t_rh_cluster = {t_dyn_cluster, t_rh_cluster} Myr\n')
        ...
        # typically we don't consider merging since it is very rare given a resonable merge radius
        if merge == True:
...
if __name__ == '__main__':
    ...
    N_cluster = 10
    Mtot_cluster = 10 | units.MSun * N_cluster
    ...
    lr9 = evolve(cluster,cloud, conv_grav,conv_sph, t_end,dt_bridge,dt_diag, sink=True, merge=False)
